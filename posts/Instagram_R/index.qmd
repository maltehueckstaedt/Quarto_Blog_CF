...
Network analysis can be applied in many different fields, such as sociology, psychology, biology, computer science, economics and political science. It can be used to visualise, quantify, compare and study networks to gain insights into the structure and function of the network.
...
The following creates an empty data frame called `df` to store information about the relationships between tagged users in Instagram posts. The code then goes through each list in the `TaggedUsers`-list and creates an edge list of all possible combinations of tagged users in each post. The edge list is then converted into a data frame containing the two tagged users and the post they appear in. This is achieved with the `as.data.frame()`-function.
...
Finally, the channel list is added to the overall data frame *df* with the `rbind`-function. The `rbind`-function adds one rows to the data frame for each instagram post entry in each iteration of the loop. In this way, we generate a data frame that contains the relationships between all tagged users in all Instagram posts.
...
![](images/IFC.png){fig-align="center" width="631"}
...
In a further step, we send the information about the number of followers and the type of peers to the nodes.
...
net_3d$nodes <- inner_join(user_info_df, net_3d$nodes, by = "name")
```
...
-   The edge thickness is indicates the intensity of two-way connections.
...
Once again, we can see that intuitively, the members of Sucuc and Bratwurst have the greatest betweenness centrality and thus have greater control over the communication in their network.
